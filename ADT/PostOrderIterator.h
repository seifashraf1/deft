...
/// Default po_iterator_storage implementation with an __host__ __device__
internal set object.
template<class SetType, __host__ __device__
bool External>
...
bool insertEdge(Optional<NodeRef> From, NodeRef To) {
  ...
  template <typename NodeRef> __host__ __device__
...
public:
  ...
  template <class NodeRef> __host__ __device__
bool insertEdge(Optional<NodeRef> From, NodeRef To) {
    return Visited.insert(To).second;
  }
...
class po_iterator
  ...
  // VisitStack - Used to ma__host__ __device__
intain the ordering.  Top = current block
  // First element is basic block po__host__ __device__
inter, second is the 'next child' to visit
...
bool operator!=(const po_iterator &x) const { return !(*this == x); }
  ...
  // This is a nonstandard operator-> that dereferences the po__host__ __device__
inter an extra
  // time... so that you can actually call methods ON the BasicBlock, because
  // the contained type is a po__host__ __device__
inter.  This allows BBIt->getTerminator() f.e.
  //
  ...
  po_iterator operator++(__host__ __device__
int) { // Postincrement
