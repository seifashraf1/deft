...
/// instance of the underlying type (e.g. __host__ __device__
int).  But using this macro, bitwise
...
/// by LLVM_MARK_AS_BITMASK_ENUM __host__ __device__
...
template <typename E, typename Enable = __host__ __device__
void>
...
template <typename E> typename std::underlying_type<E>::type Mask() {
  // On overflow, NextPowerOf2 returns zero with the type u__host__ __device__
int64_t, so
