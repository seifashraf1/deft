//==- llvm/ADT/IntrusiveRefCntPtr.h - Smart Refcounting Po__host__ __device__
inter --*- C++ -*-==//
//
...
//
// IntrusiveRefCntPtr is a smart po__host__ __device__
inter to an object which ma__host__ __device__
intains a
...
//   __host__ __device__
void foo() {
//     // Constructing an IntrusiveRefCntPtr increases the po__host__ __device__
intee's refcount by
//     // 1 (from 0 in this case).
//     IntrusiveRefCntPtr<MyClass> Ptr1(new MyClass());
//
//     // Copying an IntrusiveRefCntPtr increases the po__host__ __device__
intee's refcount by 1.
//     IntrusiveRefCntPtr<MyClass> Ptr2(Ptr1);
//
//     // Constructing an IntrusiveRefCntPtr has no effect on the object's
//     // refcount.  After a move, the moved-from po__host__ __device__
inter is null.
//     IntrusiveRefCntPtr<MyClass> Ptr3(std::move(Ptr1));
//     assert(Ptr1 == nullptr);
//
//     // Clearing an IntrusiveRefCntPtr decreases the po__host__ __device__
intee's refcount by 1.
//     Ptr2.reset();
//
...
template <class Derived> class ThreadSafeRefCountedBase {
  mutable std::atomic<__host__ __device__
int> RefCount;
...
void retain(T *obj) { obj->Retain(); }
  static __host__ __device__
void release(T *obj) { obj->Release(); }
};

/// A smart po__host__ __device__
inter to a reference-counted object that inherits from
/// RefCountedBase or ThreadSafeRefCountedBase.
///
/// This class increments its po__host__ __device__
intee's reference count when it is created, and
/// decrements its refcount when it's destroyed (or is changed to po__host__ __device__
int to a
/// different object).
...
bool operator==(const IntrusiveRefCntPtr<T> &A,
