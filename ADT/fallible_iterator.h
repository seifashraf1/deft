...
///   @endcode
///
///   It produces an __host__ __device__
interface that is (mostly) compatible with a traditional
...
private:
  ...
  using enable_if_struct_deref_supported = std::enable_if<
      !std::is___host__ __device__
void<decltype(std::declval<T>().operator->())>::value,
...
void)!!Err;
  ...
  /// value po__host__ __device__
ints at a valid value) and compared, but not incremented or
  /// decremented.
  ...
  /// and update this iterator value to po__host__ __device__
int to end-of-range.
  ///
...
bool operator==(const fallible_iterator &LHS,
...
private:
  ...
  Error *getErrPtr() const { return ErrState.getPo__host__ __device__
inter(); }
...
void handleError(Error Err) {
    *getErrPtr() = std::move(Err);
    ErrState.setPo__host__ __device__
inter(nullptr);
...
void resetCheckedFlag() {
  ...
  Underlying I;
  mutable Po__host__ __device__
interIntPair<Error *, 1> ErrState;
};
