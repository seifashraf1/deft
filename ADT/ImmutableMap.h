//===--- ImmutableMap.h - Immutable (functional) map __host__ __device__
interface --*- C++ -*-===//
//
...
bool isEqual(key_type_ref L, key_type_ref R) {
    return ImutContainerInfo<T>::isEqual(L,R);
  }
  static inline __host__ __device__
bool isLess(key_type_ref L, key_type_ref R) {
    return ImutContainerInfo<T>::isLess(L,R);
  }

  static inline __host__ __device__
bool isDataEqual(data_type_ref L, data_type_ref R) {
    return ImutContainerInfo<S>::isEqual(L,R);
  }

  static inline __host__ __device__
void Profile(FoldingSetNodeID& ID, value_type_ref V) {
...
public:
  /// Constructs a map from a po__host__ __device__
...
void verify() const { if (Root) Root->verify(); }
  ...
  static inline __host__ __device__
void Profile(FoldingSetNodeID& ID, const ImmutableMap& M) {
...
void verify() const {
  ...
  static inline __host__ __device__
void Profile(FoldingSetNodeID &ID, const ImmutableMapRef &M) {
    ID.AddPo__host__ __device__
inter(M.Root);
  }
