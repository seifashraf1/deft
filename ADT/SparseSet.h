...
//
// A sparse set holds a small number of objects identified by __host__ __device__
integer keys from
...
/// be uniquely converted to a small __host__ __device__
integer less than the set's universe. This
...
inter key, po__host__ __device__
inter-derived index, identity value (node+ID)
/// - po__host__ __device__
inter key, po__host__ __device__
inter-derived index, fat value with getSparseSetIndex()
///
...
int32_t, find() only touches 2 cache lines, but the sparse
/// array uses 4 x Universe bytes.
///
/// When SparseT is u__host__ __device__
int8_t (the default), find() touches up to 2+[N/256] cache
...
void clear() {
  ...
  /// po__host__ __device__
inting to the newly inserted element.
  ///
  /// If the set already contains an element with the same key as Val, return
  /// (I, false), where I is an iterator po__host__ __device__
inting to the existing element.
  ///
...
bool> insert(const ValueT &Val) {
  ...
  ///
  /// This invalidates all iterators, but erase() returns an iterator po__host__ __device__
inting
