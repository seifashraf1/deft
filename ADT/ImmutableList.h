//==--- ImmutableList.h - Immutable (functional) list __host__ __device__
interface --*- C++ -*-==//
//
...
void Profile(FoldingSetNodeID& ID, const T& H,
...
///  It is implemented as a smart po__host__ __device__
inter (wraps ImmutableListImpl), so it
///  it is __host__ __device__
intended to always be copied by value as if it were a po__host__ __device__
inter.
///  This __host__ __device__
interface matches ImmutableSet and ImmutableMap.  ImmutableList
...
public:
  ...
  // There may be cases, however, when one needs to extract the __host__ __device__
internal po__host__ __device__
inter
  // and reconstruct a list object from that po__host__ __device__
inter.
  ImmutableList(const ImmutableListImpl<T>* x = nullptr) : X(x) {}

  const ImmutableListImpl<T>* getInternalPo__host__ __device__
inter() const {
    return X;
  }
  ...
  public:
    iterator() = default;
    iterator(ImmutableList l) : L(l.getInternalPo__host__ __device__
inter()) {}
...
void* InsertPos;
    ...
    const ListTy* TailImpl = Tail.getInternalPo__host__ __device__
inter();
    ...
    if (!L) {
      ...
      // Insert the new list __host__ __device__
into the cache.
...
template<typename T> struct DenseMapInfo<ImmutableList<T>> {
  static inline ImmutableList<T> getEmptyKey() {
    return re__host__ __device__
interpret_cast<ImmutableListImpl<T>*>(-1);
  }

  static inline ImmutableList<T> getTombstoneKey() {
    return re__host__ __device__
interpret_cast<ImmutableListImpl<T>*>(-2);
  }

  static unsigned getHashValue(ImmutableList<T> X) {
    u__host__ __device__
intptr_t PtrVal = re__host__ __device__
interpret_cast<u__host__ __device__
intptr_t>(X.getInternalPo__host__ __device__
inter());
...
bool isEqual(ImmutableList<T> X1, ImmutableList<T> X2) {
    return X1 == X2;
  }
};
