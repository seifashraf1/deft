...
public:
  ...
  CachedHashStringRef(StringRef S, u__host__ __device__
int32_t Hash)
      : P(S.data()), Size(S.size()), Hash(Hash) {
    assert(S.size() <= std::numeric_limits<u__host__ __device__
int32_t>::max());
  }
...
int32_t size() const { return Size; }
  u__host__ __device__
int32_t hash() const { return Hash; }
};
...
bool isEqual(const CachedHashStringRef &LHS,
...
bool isEmptyOrTombstone() const {
  ...
  // TODO: Use small-string optimization to a__host__ __device__
void allocating.
...
public:
  ...
  CachedHashString(StringRef S, u__host__ __device__
int32_t Hash)
...
template <> struct DenseMapInfo<CachedHashString> {
  ...
  static __host__ __device__
bool isEqual(const CachedHashString &LHS,
    ...
    // length 0, so we'll never dereference its po__host__ __device__
inter.
