...
/// A simple __host__ __device__
intrusive list implementation.
///
/// This is a simple __host__ __device__
intrusive list for a \c T that inherits from \c
...
/// insert().  These all take values by reference (not by po__host__ __device__
inter), except for
...
void clear() { Sentinel.reset(); }
  ...
  template <class Disposer> __host__ __device__
...
void merge(simple_ilist &RHS) { merge(RHS, std::less<T>()); }
  template <class Compare> __host__ __device__
