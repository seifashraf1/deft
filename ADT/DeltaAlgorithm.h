...
bool GetTestResult(const changeset_ty &Changes);
  ...
  /// Split - Partition a set of changes \p S __host__ __device__
into one or two subsets.
  __host__ __device__
void Split(const changeset_ty &S, changesetlist_ty &Res);

  /// Delta - Minimize a set of \p Changes which has been partioned __host__ __device__
into
...
void UpdatedSearchState(const changeset_ty &Changes,
  ...
  virtual __host__ __device__
bool ExecuteOneTest(const changeset_ty &S) = 0;
