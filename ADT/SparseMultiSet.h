...
// the SparseSet.
//
// A sparse multiset holds a small number of objects identified by __host__ __device__
integer keys
...
/// array holding indexes __host__ __device__
into the dense vector.  Most of the memory is used by
...
///
/// When SparseT is u__host__ __device__
int32_t, find() only touches up to 3 cache lines, but the
/// sparse array uses 4 x Universe bytes.
///
/// When SparseT is u__host__ __device__
int8_t (the default), find() touches up to 3+[N/256] cache
/// lines, but the sparse array is 4x smaller.  N is the number of elements in
/// the set.
///
/// For sets that may grow to thousands of elements, SparseT should be set to
/// u__host__ __device__
int16_t or u__host__ __device__
int32_t.
///
...
/// the iterators don't po__host__ __device__
int to the element erased), allowing for more
/// __host__ __device__
intuitive and fast removal.
///
/// @tparam ValueT      The type of objects in the set.
/// @tparam KeyFunctorT A functor that computes an unsigned index from KeyT.
/// @tparam SparseT     An unsigned __host__ __device__
integer type. See above.
///
template<typename ValueT,
         typename KeyFunctorT = identity<unsigned>,
         typename SparseT = u__host__ __device__
int8_t>
class SparseMultiSet {
  static_assert(std::numeric_limits<SparseT>::is___host__ __device__
integer &&
                !std::numeric_limits<SparseT>::is_signed,
                "SparseT must be an unsigned __host__ __device__
integer type");

  /// The actual data that's stored, as a doubly-linked list implemented via
  /// indices __host__ __device__
into the DenseVector.  The doubly linked list is implemented
...
bool isValid() const { return Prev != INVALID; }
  ...
  /// puts a singly-linked free list __host__ __device__
into tombstone slots, allowing us quick
...
inters are to the tail of the list, allowing for efficient access to the
...
bool operator!=(const iterator_base &RHS) const {
    ...
    iterator_base operator--(__host__ __device__
int) { // postdecrement
...
void clear() {
  ...
  iterator findIndex(unsigned Idx) {
    ...
    for (unsigned i = Sparse[Idx], e = Dense.size(); i < e; i += Stride) {
      ...
      // Check that we're po__host__ __device__
inting at the correct entry and that it is the head
      // of a valid list.
...
bool contains(const KeyT &Key) const {
  ...
  ///
  /// This invalidates iterators po__host__ __device__
inting at the same entry, but erase() returns
  /// an iterator po__host__ __device__
inting to the next element in the subset's list. This makes
...
bool isBegin = B == I; !isBegin; /* empty */) {
  ///    isBegin = (--I) == B;
  ///    if (test(I))
  ///      break;
  ///    I = erase(I);
