...
//  std::any.  The idea is to provide a type-safe replacement for C's __host__ __device__
void*.
...
class Any {
  ...
  struct StorageBase {
    ...
    virtual const __host__ __device__
void *id() const = 0;
  };
...
void *id() const override { return &TypeId<T>::Id; }
...
private:
  ...
  template <class T> friend T *any_cast(Any *Value);
  template <typename T> friend __host__ __device__
