...
/// visitation order to respect the last insertion po__host__ __device__
int. This can be useful
/// when the visit order needs to be prioritized based on insertion po__host__ __device__
int
...
/// types like po__host__ __device__
inters or optionals.
///
...
/// map must support the basic DenseMap API for mapping from a T to an __host__ __device__
integer
...
bool insert(const T &X) {
  ...
  /// Insert a sequence of new elements __host__ __device__
into the PriorityWorklist.
  ...
  insert(SequenceT &&Input) {
    ...
    // First pull the input sequence __host__ __device__
into the vector as a bulk append
    // operation.
