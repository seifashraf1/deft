...
/// when required.
///
/// NOTE: This container doesn't allow you to store a null po__host__ __device__
...
public:
  ...
  using PtrUnion = Po__host__ __device__
interUnion<EltTy, VecTy *>;
...
public:
  ...
  TinyPtrVector &operator=(const TinyPtrVector &RHS) {
    ...
    // Try to squeeze __host__ __device__
into the single slot. If it won't fit, allocate a copied
    // vector.
...
bool empty() const {
    ...
    // contains a po__host__ __device__
inter to an empty vector.
  ...
  EltTy operator[](unsigned i) const {
    assert(!Val.isNull() && "can't index __host__ __device__
...
void clear() {
  ...
  iterator erase(iterator I) {
    ...
    if (Val.template is<EltTy>()) {
      ...
      // benefit to collapsing back to a po__host__ __device__
inter
      return Vec->erase(I);
    }
    return end();
