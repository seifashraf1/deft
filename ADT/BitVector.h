...
public:
  const_set_bits_iterator_impl(const BitVectorT &Parent, __host__ __device__
int Current)
  ...
  const_set_bits_iterator_impl operator++(__host__ __device__
int) {
    auto Prev = *this;
    advance();
    return Prev;
...
bool() const {
  ...
  explicit BitVector(unsigned s, __host__ __device__
bool t = false) : Size(s) {
...
bool test(const BitVector &RHS) const {
  ...
  BitVector &operator>>=(unsigned N) {
    ...
    // of the bits from the next word and or them __host__ __device__
into this word while
  ...
  BitVector &operator<<=(unsigned N) {
    ...
    // When the shift size is not a multiple of the word size, then we have
    // a tricky situation where each word in succession needs to extract some
    // of the bits from the previous word and or them __host__ __device__
into this word while
    // shifting this word to make room for the new bits.  This has to be done
...
void swap(BitVector &RHS) {
  ...
  //===--------------------------------------------------------------------===//
  //
  // These methods all operate on arrays of u__host__ __device__
int32_t, each holding 32 bits. The
...
private:
  ...
  /// turn this __host__ __device__
into 0x00000000 0x00000000 0xBBBBAAAA 0xDDDDFFFF by using a
  /// memmove which moves right, not left.
  __host__ __device__
void wordShl(u__host__ __device__
int32_t Count) {
    if (Count == 0)
      return;

    u__host__ __device__
int32_t NumWords = NumBitWords(Size);
  ...
  ///
  __host__ __device__
void wordShr(u__host__ __device__
int32_t Count) {
    if (Count == 0)
      return;
...
int32_t NumWords = NumBitWords(Size);
  ...
  __host__ __device__
int next_unset_in_word(__host__ __device__
int WordIndex, BitWord Word) const {
  ...
  __host__ __device__
void set_unused_bits(__host__ __device__
bool t = true) {
...
void grow(unsigned NewSize) {
  ...
  __host__ __device__
void init_words(MutableArrayRef<BitWord> B, __host__ __device__
bool t) {
    if (B.size() > 0)
      memset(B.data(), 0 - (__host__ __device__
int)t, B.size() * sizeof(BitWord));
  }
