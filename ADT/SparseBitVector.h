...
/// list of SparseBitVectorElements.  We ma__host__ __device__
intain a po__host__ __device__
inter to the last
...
/// kept up to date.  They are also significantly more memory __host__ __device__
intensive.
...
intersects(const SparseBitVectorElement &RHS) const {
    for (unsigned i = 0; i < BITWORDS_PER_ELEMENT; ++i) {
  ...
  __host__ __device__
bool __host__ __device__
intersectWith(const SparseBitVectorElement &RHS,
                     __host__ __device__
bool &BecameZero) {
    __host__ __device__
bool changed = false;
...
bool allzero = true;
  ...
  __host__ __device__
bool __host__ __device__
intersectWithComplement(const SparseBitVectorElement &RHS,
                               __host__ __device__
bool &BecameZero) {
    __host__ __device__
bool changed = false;
...
bool allzero = true;
  ...
  // Three argument version of __host__ __device__
intersectWithComplement that __host__ __device__
intersects
  // RHS1 & ~RHS2 __host__ __device__
into this element
  __host__ __device__
void __host__ __device__
intersectWithComplement(const SparseBitVectorElement &RHS1,
...
inter to our current Element. This has no visible effect on the external
  ...
  ElementListIter FindLowerBoundImpl(unsigned ElementIndex) const {
    ...
    // get the begin/end in the case where 'this' is const. To a__host__ __device__
void duplication
...
bool end = false):BitVector(RHS) {
    ...
    // Postincrement.
    inline SparseBitVectorIterator operator++(__host__ __device__
int) {
...
bool changed = false;
    ...
    // Loop through, __host__ __device__
intersecting as we go, erasing elements when necessary.
    while (Iter2 != RHS.Elements.end()) {
      ...
      if (Iter1->index() > Iter2->index()) {
        ...
        __host__ __device__
bool BecameZero;
        changed |= Iter1->__host__ __device__
intersectWith(*Iter2, BecameZero);
        if (BecameZero) {
  ...
  __host__ __device__
bool __host__ __device__
intersectWithComplement(const SparseBitVector &RHS) {
    if (this == &RHS) {
      if (!empty()) {
        clear();
        return true;
...
bool BecameZero;
        changed |= Iter1->__host__ __device__
intersectWithComplement(*Iter2, BecameZero);
        if (BecameZero) {
          ElementListIter IterTmp = Iter1;
          ++Iter1;
          Elements.erase(IterTmp);
  ...
  __host__ __device__
bool __host__ __device__
intersectWithComplement(const SparseBitVector<ElementSize> *RHS) const {
...
intersectWithComplement.
  //  Result of RHS1 & ~RHS2 is stored __host__ __device__
into this bitmap.
  __host__ __device__
void __host__ __device__
intersectWithComplement(const SparseBitVector<ElementSize> &RHS1,
...
bool BecameZero = false;
        ...
        Elements.back().__host__ __device__
intersectWithComplement(*Iter1, *Iter2, BecameZero);
        if (BecameZero)
          Elements.pop_back();
        ++Iter1;
        ++Iter2;
...
intersectWithComplement(*RHS1, *RHS2);
  ...
  __host__ __device__
bool __host__ __device__
intersects(const SparseBitVector<ElementSize> *RHS) const {
    return __host__ __device__
intersects(*RHS);
  }
...
intersects(const SparseBitVector<ElementSize> &RHS) const {
    ...
    // Loop through, __host__ __device__
intersecting stopping when we hit bits in common.
    while (Iter2 != RHS.Elements.end()) {
      if (Iter1 == Elements.end())
        return false;
...
bool operator |=(SparseBitVector<ElementSize> &LHS,
...
// Convenience functions for infix union, __host__ __device__
intersection, difference operators.
