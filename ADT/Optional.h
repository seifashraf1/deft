...
void *)std::addressof(value)) T(std::forward<Args>(args)...);
    hasVal = true;
  }
...
void *)std::addressof(value)) T(y);
  ...
  OptionalStorage &operator=(T &&y) {
    if (hasValue()) {
      ...
      ::new ((__host__ __device__
void *)std::addressof(value)) T(std::move(y));
      hasVal = true;
    }
    return *this;
  }
  ...
  OptionalStorage &operator=(OptionalStorage const &other) {
    if (other.hasValue()) {
      if (hasValue()) {
        ...
        ::new ((__host__ __device__
void *)std::addressof(value)) T(other.value);
        hasVal = true;
      }
    } else {
      reset();
...
public:
  ...
  template <typename... ArgTypes> __host__ __device__
...
void reset() { Storage.reset(); }
  ...
  const T *getPo__host__ __device__
inter() const { return &Storage.getValue(); }
  T *getPo__host__ __device__
inter() { return &Storage.getValue(); }
  const T &getValue() const LLVM_LVALUE_FUNCTION { return Storage.getValue(); }
  T &getValue() LLVM_LVALUE_FUNCTION { return Storage.getValue(); }

  explicit operator __host__ __device__
bool() const { return hasValue(); }
  __host__ __device__
bool hasValue() const { return Storage.hasValue(); }
  const T *operator->() const { return getPo__host__ __device__
inter(); }
  T *operator->() { return getPo__host__ __device__
inter(); }
