...
template <typename T>
struct negation : std::__host__ __device__
...
struct conjunction<B1, Bn...>
    : std::conditional<__host__ __device__
bool(B1::value), conjunction<Bn...>, B1>::type {};

template <typename T> struct make_const_ptr {
  using type =
      typename std::add_po__host__ __device__
inter<typename std::add_const<T>::type>::type;
};
...
class function_ref<Ret(Params...)> {
  Ret (*callback)(__host__ __device__
intptr_t callable, Params ...params) = nullptr;
  __host__ __device__
intptr_t callable;

  template<typename Callable>
  static Ret callback_fn(__host__ __device__
intptr_t callable, Params ...params) {
    return (*re__host__ __device__
interpret_cast<Callable*>(callable))(
...
public:
  ...
  function_ref(Callable &&callable,
      ...
      : callback(callback_fn<typename std::remove_reference<Callable>::type>),
        callable(re__host__ __device__
...
bool empty(const T &RangeOrContainer) {
...
bool value = sizeof(test<Ty>(nullptr)) == sizeof(yes);
};
...
/// iterator's reference type and returns a __host__ __device__
bool. When incrementing or
...
///   __host__ __device__
int A[] = { 1, 2, 3, 4 };
///   auto R = make_filter_range(A, [](__host__ __device__
int N) { return N % 2 == 1; });
///   // R contains { 1, 3 }.
/// \endcode
///
...
bool is_bidirectional> struct fwd_or_bidi_tag_impl {
...
intended primarily to be used with range based for loops and some range
/// algorithms.
///
/// The iterator isn't quite an `OutputIterator` or an `InputIterator` but
/// somewhere between them. The constra__host__ __device__
ints of these iterators are:
///
...
interT = typename std::iterator_traits<WrappedIteratorT>::po__host__ __device__
inter;
...
bool test(const zip_shortest<Iters...> &other, index_sequence<Ns...>) const {
    return all_of(std::initializer_list<__host__ __device__
bool>{std::get<Ns>(this->iterators) !=
                                              std::get<Ns>(other.iterators)...},
                  identity<__host__ __device__
bool>{});
  }
...
public:
  ...
  using po__host__ __device__
inter = typename iterator::po__host__ __device__
inter;
...
bool test(const zip_longest_iterator<Iters...> &other,
    ...
    return llvm::any_of(
        std::initializer_list<__host__ __device__
bool>{std::get<Ns>(this->iterators) !=
                                    std::get<Ns>(other.iterators)...},
        identity<__host__ __device__
bool>{});
...
///
/// This can concatenate different iterators, even with different types, __host__ __device__
into
/// a single iterator provided the value types of all the concatenated
/// iterators expose `reference` and `po__host__ __device__
inter` types that can be converted to
/// `ValueT &` and `ValueT *` respectively. It doesn't support more
/// __host__ __device__
interesting/customized po__host__ __device__
inter or reference types.
///
/// Currently this only supports forward or higher iterator categories as
/// inputs and always exposes a forward iterator __host__ __device__
interface.
...
class concat_iterator
  ...
  template <size_t Index> __host__ __device__
bool incrementHelper() {
  ...
  template <size_t... Ns> __host__ __device__
...
bool (concat_iterator::*IncrementHelperFns[])() = {
  ...
  template <size_t... Ns> ValueT &get(index_sequence<Ns...>) const {
    ...
    llvm_unreachable("Attempted to get a po__host__ __device__
inter from an end concat iterator!");
  }
...
/// are passed __host__ __device__
into the constructor such that we can use it as part of range
/// based for loops.
...
/// Represents a compile-time sequence of __host__ __device__
integers.
template <class T, T... I> struct __host__ __device__
integer_sequence {
  using value_type = T;
...
template <size_t... I>
struct index_sequence : __host__ __device__
...
/// Creates a compile-time __host__ __device__
integer sequence for a parameter pack.
template <class... Ts>
...
template<typename T>
inline __host__ __device__
int array_pod_sort_comparator(const __host__ __device__
void *P1, const __host__ __device__
void *P2) {
  if (std::less<T>()(*re__host__ __device__
interpret_cast<const T*>(P1),
                     *re__host__ __device__
interpret_cast<const T*>(P2)))
    return -1;
  if (std::less<T>()(*re__host__ __device__
interpret_cast<const T*>(P2),
                     *re__host__ __device__
...
/// get_array_pod_sort_comparator - This is an __host__ __device__
internal helper function used to
/// get type deduction of T right.
template<typename T>
inline __host__ __device__
int (*get_array_pod_sort_comparator(const T &))
...
template <typename Container> inline __host__ __device__
...
/// For a container of po__host__ __device__
inters, deletes the po__host__ __device__
inters and then clears the
/// container.
template<typename Container>
__host__ __device__
void DeleteContainerPo__host__ __device__
inters(Container &C) {
  for (auto V : C)
    delete V;
  C.clear();
...
/// In a container of pairs (usually a map) whose second element is a po__host__ __device__
inter,
...
auto partition_po__host__ __device__
int(R &&Range, Predicate P) -> decltype(adl_begin(Range)) {
  return std::partition_po__host__ __device__
int(adl_begin(Range), adl_end(Range), P);
}
...
/// Example:
///
///     auto p = make_unique<__host__ __device__
int>();
///     auto p = make_unique<std::tuple<__host__ __device__
...
///
///     auto p = make_unique<__host__ __device__
int[]>(2); // value-initializes the array with 0's.
template <class T>
...
struct FreeDeleter {
  __host__ __device__
void operator()(__host__ __device__
void* v) {
    ::free(v);
  }
};
...
/// A functor like C++14's std::less<__host__ __device__
void> in its absence.
struct less {
...
/// A functor like C++14's std::equal<__host__ __device__
void> in its absence.
struct equal {
  template <typename A, typename B> __host__ __device__
bool operator()(A &&a, B &&b) const {
...
class enumerator_iter
    : public iterator_facade_base<
          ...
          typename std::iterator_traits<IterOfRange<R>>::po__host__ __device__
inter,
          typename std::iterator_traits<IterOfRange<R>>::reference> {
...
/// Returns a raw po__host__ __device__
inter that represents the same address as the argument.
///
...
///
/// The std::po__host__ __device__
inter_traits<>::to_address(p) variations of these overloads has
