...
// like this:
//
//  ScopedHashTable<__host__ __device__
int, __host__ __device__
int> HT;
//  {
//    ScopedHashTableScope<__host__ __device__
int, __host__ __device__
int> Scope1(HT);
//    HT.insert(0, 0);
//    HT.insert(1, 1);
...
public:
  ...
  template <typename AllocatorTy> __host__ __device__
...
bool operator!=(const ScopedHashTableIterator &RHS) const {
  ...
  ScopedHashTableIterator operator++(__host__ __device__
int) {        // Postincrement
...
void insert(const K &Key, const V &Val) {
  ...
  /// (possibly not the current) scope.  While it is ok to insert __host__ __device__
into a scope
