...
public:
  ...
  static bool prepareSummaryState(llvm::AbstractCallSite cs,
...
public:
  ...
  __global__ void analyzeCallKernel(llvm::AbstractCallSite cs, State& state, const Context& context) {
    Context newContext;
  ...
  void
  analyzeCall(llvm::AbstractCallSite cs, State &state, const Context& context) {
    Context newContext;
    if (newContext.size() > 0) {
      std::copy(context.begin() + 1, context.end(), newContext.begin());
      newContext.back() = cs.getInstruction();
